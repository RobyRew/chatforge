...
- [ ] Next: **CH-5** encrypted attachments (`blob.ts`→MinIO; reference `{objectKey, wrappedKey}` inside the MLS message). Also pending: rich statuses (Spotify/Discord-style presence), device-at-rest sealing of chat group state, "linked vault" banner in the live thread, Argon2id upgrade for the vault KDF, api-client codegen, Meta/Discord importers.
