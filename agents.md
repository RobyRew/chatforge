...
- [ ] Next: **CH-5** encrypted attachments (`blob.ts`→MinIO; reference `{objectKey, wrappedKey}` inside the MLS message). Also pending: **Vault Conversations** (saved imported chats linked to live DMs), rich statuses (Spotify/Discord-style), device-at-rest sealing of chat group state (vault), api-client codegen, Meta/Discord importers.
