...
describe('api', () => {
  ...
  it('rejects unauthenticated server-side conversion', async () => {
    const res = await post('/api/convert', { fileName: 'x.txt', contentBase64: '', target: 'json' });
  ...
  it('runs server-side conversion for an authorized user (hybrid path)', async () => {
    ...
    const res = await post('/api/convert', { fileName: 'chat.txt', contentBase64, target: 'telegram-json' }, 'user-token');
    expect(res.status).toBe(200);
  ...
  it('enforces RBAC on the audit log', async () => {
    expect((await get('/api/admin/audit', 'user-token')).status).toBe(403);
    expect((await get('/api/admin/audit', 'owner-token')).status).toBe(200);
  });

  it('lets an owner grant a role and an admin toggle a feature flag', async () => {
    const grant = await post('/api/admin/users/u_user/role', { role: 'moderator' }, 'owner-token');
    expect(grant.status).toBe(200);
    expect(((await grant.json()) as { user: { role: string } }).user.role).toBe('moderator');

    const flag = await post('/api/admin/flags/chat', { enabled: true }, 'owner-token');
